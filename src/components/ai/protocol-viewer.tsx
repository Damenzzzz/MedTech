...
export function ProtocolViewer({
  ...
  return (
    <>
      <button type="button" className="fixed inset-0 z-[60] bg-[rgba(16,32,43,0.5)]" onClick={onClose} aria-label={t('closeProtocol')} />
      <aside className="fixed inset-y-0 right-0 z-[70] flex w-full max-w-3xl flex-col border-l border-[var(--glass-border)] bg-[var(--surface)] text-[var(--text-primary)] shadow-2xl">
        <header className="flex items-start justify-between gap-4 border-b border-[var(--border-color)] p-5">
          <div className="flex min-w-0 items-start gap-3">
            <BookOpen className="mt-0.5 shrink-0 text-[#1F6FEB]" size={20} />
          ...
          <button type="button" onClick={onClose} className="rounded-lg p-2 text-[var(--text-tertiary)] hover:bg-[#F4F7FB] hover:text-[var(--text-primary)]" aria-label={t('close')}>
            <X size={18} />
          </button>
        </header>

        <div className="flex-1 overflow-y-auto p-5 sm:p-7">
          {loading && <div className="flex items-center gap-2 text-sm text-[var(--text-tertiary)]"><Loader2 className="animate-spin" size={17} />{t('loading')}</div>}
          {error && <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div>}
          {document && (
            <article className="whitespace-pre-wrap text-sm leading-7 text-[var(--text-secondary)]">
              {match ? (
                <>
                  {document.text.slice(0, match.start)}
                  <mark ref={markRef} className="rounded bg-[#F3CA8D] px-0.5 text-[#4A2F0E]">{document.text.slice(match.start, match.end)}</mark>
