...
def test_strict_context_drops_globally_valid_but_unretrieved_code(monkeypatch):
    monkeypatch.setenv("STRICT_CONTEXT_ICD", "1")
    ...
    assert [item["icd10_code"] for item in result] == ["A04.6"]


def test_strict_context_expands_prefix_only_inside_retrieved_codes(monkeypatch):
    monkeypatch.setenv("STRICT_CONTEXT_ICD", "1")
    postprocessor.ALL_VALID_ICD_CODES.clear()
...
def test_icd_parser_normalizes_cyrillic_ocr_variants():
    text = "Код по МКБ: К 22,0 Ахалазия; С 22.2 Гепатобластома; F32.1 Депрессия."

    assert extract_icd_codes(text) == ["K22.0", "C22.2", "F32.1"]
    assert extract_icd_labels(text)["K22.0"] == "Ахалазия"


def test_context_selection_preserves_reranker_order():
    ...
    assert [chunk.protocol_id for chunk in selected] == ["p1", "p2"]


def test_context_selection_limits_duplicate_protocol_titles():
    chunks = [
        Chunk(
    ...
    assert len(select_top_chunks(chunks)) == 2


def test_strategy_coverage_promotes_icd_candidate():
    chunks = [
    ...
    assert chunks[7] in covered[:7]


def test_faithfulness_filter_does_not_turn_protocol_criteria_into_patient_facts():
    patient_text = (
    ...
    assert [item["finding"] for item in result[0]["supporting_findings"]] == ["АД 170/110"]
    assert any("протеинурия" in item for item in result[0]["missing_findings"])
    assert "протеинурия" not in result[0]["explanation"].split("Факты пациента:", 1)[-1].split(".", 1)[0]


def test_faithfulness_filter_drops_broad_duplicate_after_specific_code():
    result = postprocessor.ground_diagnoses_to_patient(
        [
            {
                "diagnosis": "Тяжелая преэклампсия",
    ...
    assert [item["icd10_code"] for item in result] == ["O14.1"]


def test_malformed_json_parser_recovers_diagnosis_code_pairs():
    raw = '{"diagnoses":[{"diagnosis":"Синдром HELLP","icd10_code":"O14.2","confidence":"high",'

    assert postprocessor.parse_diagnosis_json(raw) == [
        {"diagnosis": "Синдром HELLP", "icd10_code": "O14.2", "confidence": "high"}
    ]


def test_hellp_fallback_prefers_specific_obstetric_code():
    chunk = Chunk(
        chunk_id="hellp",
        protocol_id="hellp",
    ...
    assert _preferred_codes_for_chunk(chunk) == ["O14.2"]


def test_obstetric_unknowns_are_not_shown_as_support_when_absent():
    ...
    assert [item["icd10_code"] for item in result] == ["O14.2", "O13"]
    ...
    assert "протеинур" not in support_text
    assert "гемолиз" not in support_text
    assert "протеинур" in missing_text
    assert "гемолиз" in missing_text
    assert result[0]["why_this_diagnosis"]


def test_severe_preeclampsia_ranks_before_hellp_without_hemolysis():
    result = postprocessor.ground_diagnoses_to_patient(
        [
            {
                "diagnosis": "Синдром HELLP",
    ...
    assert [item["icd10_code"] for item in result] == ["O14.1", "O14.2"]
    assert result[1]["confidence"] == "medium"


def test_hellp_ranks_before_preeclampsia_when_hemolysis_is_confirmed():
    result = postprocessor.ground_diagnoses_to_patient(
        [
            {
                "diagnosis": "Тяжелая преэклампсия",
    ...
    assert [item["icd10_code"] for item in result] == ["O14.2", "O14.1"]
    assert result[0]["confidence"] == "high"


def test_short_why_is_generated_from_grounded_facts():
    result = postprocessor.ground_diagnoses_to_patient(
        [
            {
                "diagnosis": "Тяжелая преэклампсия",
    ...
    assert "АД 170/110" in result[0]["why_this_diagnosis"]
    assert "Протеинурия не указана" in result[0]["why_this_diagnosis"]


def test_hellp_support_prioritizes_liver_platelets_and_ruq_pain():
    result = postprocessor.ground_diagnoses_to_patient(
        [
            {
                "diagnosis": "HELLP-синдром",
    ...
    joined = " ".join(facts)
    assert "алт" in joined or "аст" in joined
    assert "тромбоцит" in joined
    assert "правом подреберье" in joined


def test_refine_preserves_previous_rag_candidates_on_noisy_input():
    previous = {
        "diagnoses": [
    ...
    assert [item["icd10_code"] for item in stabilized["diagnoses"]] == ["O14.1", "O14.2"]


def test_refine_appends_lost_previous_candidate_after_rerank():
    previous = {
        "diagnoses": [
            {"rank": 1, "diagnosis": "Тяжелая преэклампсия", "icd10_code": "O14.1", "explanation": ""},
            {"rank": 2, "diagnosis": "HELLP-синдром", "icd10_code": "O14.2", "explanation": ""},
    ...
    assert [item["icd10_code"] for item in stabilized["diagnoses"]] == ["O14.2", "O14.1"]
    assert "Сохранено из первичного RAG-кандидата" in stabilized["diagnoses"][1]["missing_findings"][0]
